...
class ThreadView(BaseView):
    ...
    QUERY_DESCRIPTION_PRODUCT = "The keyword product to filter the threads"
    ...
    async def post_v1(
        ...
        UserDataHandler.validate_email_match(user_email=user_data.email,
                                             requested_by=create_thread_request.requested_by)
        try:
    ...
    async def get_v2(
        ...
        try:
            thread_service = cls._get_thread_service(connection_handler)
            chat_threads = await cls._fetch_threads(thread_service, thread_query_params, org_id=user_data.orgId)
            return cls.construct_success_response(data=chat_threads)
        except Exception as exp:
            return cls.construct_error_response(exp)

    @staticmethod
    async def _fetch_threads(thread_service, thread_query_params, org_id: Optional[str] = None):
...
class ThreadMessageView(BaseView):
    # V1 API Methods
    ERROR_CODE_GET_THREAD_MESSAGES = 4003
    ...
    async def post_v1(
        ...
        UserDataHandler.validate_email_match(user_email=user_data.email,
                                             requested_by=create_message_request.requested_by)
        try:
            thread_service = cls._get_thread_service(connection_handler)
    ...
    async def put_v1(
        ...
        UserDataHandler.validate_email_match(user_email=user_data.email,
                                             requested_by=update_message_request.requested_by)
        try:
            thread_service = cls._get_thread_service(connection_handler)
    ...
    async def get_v1(
        ...
        try:
            ...
            if not thread_messages:
                return cls.construct_error_response("No Thread Messages Found", code=cls.ERROR_CODE_GET_THREAD_MESSAGES)
