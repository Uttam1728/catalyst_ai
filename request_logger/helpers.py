...
from utils.connection_handler import cerebrum_write_connection_handler_context


async def log_tokens(user, request_type, tokens_data, url, header="", body="", model=""):
    try:
        async with cerebrum_write_connection_handler_context() as connection_handler:
